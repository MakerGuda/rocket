    ...
    public void execute(CommandLine commandLine, Options options, RPCHook rpcHook) throws SubCommandException {
        ...
        try {
            ...
            if (commandLine.hasOption('r')) {
                resources = Arrays.stream(StringUtils.split(commandLine.getOptionValue('r'), ',')).map(StringUtils::trim).collect(Collectors.toList());
            }
            List<String> actions = null;
            if (commandLine.hasOption('a')) {
                actions = Arrays.stream(StringUtils.split(commandLine.getOptionValue('a'), ',')).map(StringUtils::trim).collect(Collectors.toList());
            }
            List<String> sourceIps = null;
            if (commandLine.hasOption('i')) {
                sourceIps = Arrays.stream(StringUtils.split(commandLine.getOptionValue('i'), ',')).map(StringUtils::trim).collect(Collectors.toList());
            }

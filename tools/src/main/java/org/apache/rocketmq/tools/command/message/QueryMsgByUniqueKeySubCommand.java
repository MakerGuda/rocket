...
public class QueryMsgByUniqueKeySubCommand implements SubCommand {
    ...
    public static void queryById(final DefaultMQAdminExt admin, final String topic, final String msgId, final boolean showAll) throws MQClientException, InterruptedException, IOException {
    ...
    private static void showMessage(final DefaultMQAdminExt admin, MessageExt msg, int index) throws IOException {
        ...
        System.out.printf(strFormat, "Properties:", msg.getProperties() != null ? msg.getProperties().toString() : "");
    ...
    private static String createBodyFile(MessageExt msg, int index) throws IOException {
        ...
        try {
            ...
            if (index > 0) {
                bodyTmpFilePath.append("_").append(index);
            }
            dos = new DataOutputStream(Files.newOutputStream(Paths.get(bodyTmpFilePath.toString())));

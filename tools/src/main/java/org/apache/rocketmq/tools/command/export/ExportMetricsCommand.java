...
public class ExportMetricsCommand implements SubCommand {
    ...
    private Map<String, Object> getRuntimeVersion(DefaultMQAdminExt defaultMQAdminExt, SubscriptionGroupWrapper subscriptionGroupWrapper) {
        ...
        for (Map.Entry<String, SubscriptionGroupConfig> entry : subscriptionGroupWrapper.getSubscriptionGroupTable().entrySet()) {
            try {
                ConsumerConnection cc = defaultMQAdminExt.examineConsumerConnectionInfo(entry.getValue().getGroupName());
    ...
    private Map<String, Object> getRuntimeQuota(KVTable kvTable, DefaultMQAdminExt defaultMQAdminExt, String brokerAddr, Map<String, Double> totalTpsMap, Map<String, Long> totalOneDayNumMap, SubscriptionGroupWrapper subscriptionGroupWrapper) throws RemotingException, MQBrokerException, InterruptedException, MQClientException {
        TopicConfigSerializeWrapper topicConfigSerializeWrapper = defaultMQAdminExt.getUserTopicConfig(brokerAddr, false, 10000);
        BrokerStatsData transStatsData = null;
        try {
            transStatsData = defaultMQAdminExt.viewBrokerStatsData(brokerAddr, Stats.TOPIC_PUT_NUMS, TopicValidator.RMQ_SYS_TRANS_HALF_TOPIC);
        } catch (MQClientException ignore) {
        }
        BrokerStatsData scheduleStatsData = null;
        try {
            scheduleStatsData = defaultMQAdminExt.viewBrokerStatsData(brokerAddr, Stats.TOPIC_PUT_NUMS, TopicValidator.RMQ_SYS_SCHEDULE_TOPIC);
        } catch (MQClientException ignore) {
        }
        ...
        long normalOneDayInNum = Long.parseLong(kvTable.getTable().get("msgPutTotalTodayMorning")) - Long.parseLong(kvTable.getTable().get("msgPutTotalYesterdayMorning"));
        long normalOneDayOutNum = Long.parseLong(kvTable.getTable().get("msgGetTotalTodayMorning")) - Long.parseLong(kvTable.getTable().get("msgGetTotalYesterdayMorning"));

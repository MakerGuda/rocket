...
public class ClusterListSubCommand implements SubCommand {
    ...
    private void printClusterMoreStats(final Set<String> clusterNames, final DefaultMQAdminExt defaultMQAdminExt, final ClusterInfo clusterInfo) {
        ...
        for (String clusterName : clusterNames) {
            ...
            for (String brokerName : brokerNameTreeSet) {
                ...
                if (brokerData != null) {
                    for (Map.Entry<Long, String> next1 : brokerData.getBrokerAddrs().entrySet()) {
                        long inTotalYest = 0;
                        long outTotalYest = 0;
                        long inTotalToday = 0;
    ...
    private void printClusterBaseInfo(final Set<String> clusterNames, final DefaultMQAdminExt defaultMQAdminExt, final ClusterInfo clusterInfo) {
            ...
            for (String brokerName : brokerNameTreeSet) {
                ...
                if (brokerData != null) {
                    for (Map.Entry<Long, String> next1 : brokerData.getBrokerAddrs().entrySet()) {
                        ...
                        double hour = 0.0;
                        double space = 0.0;
                        if (earliestMessageTimeStamp != null && !earliestMessageTimeStamp.isEmpty()) {
                            long mills = System.currentTimeMillis() - Long.parseLong(earliestMessageTimeStamp);
                            hour = mills / 1000.0 / 60.0 / 60.0;
                        }
                        if (commitLogDiskRatio != null && !commitLogDiskRatio.isEmpty()) {

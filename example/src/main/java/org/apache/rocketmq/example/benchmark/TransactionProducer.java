...
public class TransactionProducer {
    ...
    public static void main(String[] args) throws MQClientException, UnsupportedEncodingException {
        ...
        executorService.scheduleAtFixedRate(new TimerTask() {
            private void printStats() {
                if (snapshotList.size() >= 10) {
                    ...
                    System.out.printf("Current Time: %s | Send TPS: %5d | Max RT(ms): %5d | AVG RT(ms): %3.1f | Send Failed: %d | Check: %d | UnexpectedCheck: %d | DuplicatedCheck: %d%n", UtilAll.timeMillisToHumanString2(System.currentTimeMillis()), sendTps, statsBenchmark.getSendMessageMaxRT().get(), averageRT, failCount, checkCount, unexpectedCheck, dupCheck);
        ...
        final TransactionMQProducer producer = new TransactionMQProducer("benchmark_transaction_producer", rpcHook, config.msgTraceEnable, null);
...
class TransactionListenerImpl implements TransactionListener {
    ...
    public LocalTransactionState checkLocalTransaction(MessageExt msg) {
        ...
        if (msgMeta.sendResult != LocalTransactionState.UNKNOWN) {
            System.out.printf("%s unexpected check: msgId=%s,txId=%s,checkTimes=%s,sendResult=%s\n", new SimpleDateFormat("HH:mm:ss,SSS").format(new Date()), msg.getMsgId(), msg.getTransactionId(), msg.getUserProperty(MessageConst.PROPERTY_TRANSACTION_CHECK_TIMES), msgMeta.sendResult.toString());
        ...
        for (int i = 0; i < times - 1; i++) {
            ...
            if (s != LocalTransactionState.UNKNOWN) {
                System.out.printf("%s unexpected check: msgId=%s,txId=%s,checkTimes=%s,sendResult,lastCheckResult=%s\n", new SimpleDateFormat("HH:mm:ss,SSS").format(new Date()), msg.getMsgId(), msg.getTransactionId(), msg.getUserProperty(MessageConst.PROPERTY_TRANSACTION_CHECK_TIMES), s);
                statBenchmark.getUnexpectedCheckCount().increment();
                return s;
            }
        }
